...
@Component({
    ...
    styleUrls: [ './filter-multi-select.scss' ],
})
export class FilterMultiSelectComponent implements OnInit {
    ...
    @Input()
    public notifyOnItemSelect: boolean = false;
    ...
    public set isActive(val: boolean) {
        if (this.isDropdownActive !== val) {
            this.isDropdownActive = val;
            if (!this.isDropdownActive && !this.notifyOnItemSelect) {
                this.onApply();
            }
        }
    }
